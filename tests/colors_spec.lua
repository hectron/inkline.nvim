...
describe("style-specific colors", function()
  it("modern uses purple for properties", function()
    local colors, opts = colors_module.setup({ style = "modern" })
    local property_mapping = require("inkline.colors.property_mapping")
    local prop_color = property_mapping.get_property_color("modern", colors)
    assert(colors.purple == prop_color)
  end)
  ...
  it("classic uses teal for properties", function()
    local colors, opts = colors_module.setup({ style = "classic" })
    local property_mapping = require("inkline.colors.property_mapping")
    local prop_color = property_mapping.get_property_color("classic", colors)
    assert(colors.teal == prop_color)
  end)
